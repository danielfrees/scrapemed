"""
Test ScrapeMed's validate module.
"""
...
def test_xml_validation():
    ...
    (1) Validation function does not alter XML other than html unescaping of
        non-markup-significant encodings
    (2) Warnings and exceptions for unknown PMC XML/HTML are raised correctly
    (3) Capitalization doesn't matter when validating XML
        (Other than proper XML syntax & symmetry)
    ...
    # Check that lxml parsing & validation does not modify xml
    # (other than prologs, comments/markup, and html unescaping)

    # NOTE: test was manually modified to have the same prolog,markup, and
    # have html unescaped (auto performed by lxml)

    # I merely want to check that the xml contents are not
    # manipulated by validator, lxml library
    ...
    # write to file again and the test xml should be unmodified
    # by validation other than html unescaping
    XML_TEST.write(
        os.path.join(path_to_testdata, "should_be_unmodified_test.xml"),
        xml_declaration=True,
        encoding="utf-8",
    )
    assert filecmp.cmp(
        os.path.join(path_to_testdata, "test_html_unescaped.xml"),
        os.path.join(path_to_testdata, "should_be_unmodified_test.xml"),
        shallow=True,
    )

    # check that capitalization breaks the DTD validations
    XML_WEIRD_CAPS = _get_xml_from_file(
        os.path.join(path_to_testdata, "weird_caps.xml")
    )
    assert not _validate.validate_xml(XML_WEIRD_CAPS)

    # check that accepted wildcard values don't matter when
    # validating otherwise valid xml
    XML_WITH_WILDCARDS = _get_xml_from_file(
        os.path.join(path_to_testdata, "test_wildcarding_1.xml")
    )
    assert _validate.validate_xml(XML_WITH_WILDCARDS)

    # Check that cleaning html styling does not mess with validation of xml
    xml_test_text = ET.tostring(XML_TEST, encoding="unicode")
    styling_removed_tree = scrape.xml_tree_from_string(
        xml_test_text, strip_text_styling=True
    )
    ...
    # check that validation works on sample entrez download
    # PMCID = 7067710 (straight from source)
...
def _generate_nms_html_unescaped_file(filename: str) -> None:
    ...
    with open(filename, "r") as f:
        ...
        # REMOVE HTML ENCODINGS (ie: "&xA9;" -> "©")
        # OTHER THAN MARKUP-SIGNIFICANT HTML ENCODINGS
        """
